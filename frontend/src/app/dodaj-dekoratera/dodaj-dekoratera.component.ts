...
@Component({
  selector: 'app-dodaj-dekoratera',
  templateUrl: './dodaj-dekoratera.component.html',
  styleUrls: ['./dodaj-dekoratera.component.css']
})
export class DodajDekorateraComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.firmaService.dohvatiFirme().subscribe(arg => this.nizFirmi = arg);
  ...
  dodajVlasnika(){
    ...
    this.userService.registracijaKorisnika(this.korIme, this.lozinka, this.ime, this.prezime, this.pol,
      this.adresa, this.kontaktTelefon, this.imejl, this.slika, this.kartica, "dekorater", "Odobren"
    ).subscribe(arg => {
      if(arg == null){
    ...
    if(this.zaposlen == "")return;
    this.firmaService.dodajZaposlenog(this.korIme, this.zaposlen).subscribe();
